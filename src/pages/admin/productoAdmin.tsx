...
const AdminCategorySection = ({ categoryName }: { categoryName: string }) => {
    ...
    // Generar ID seguro para el scroll
    ...
    const handleDelete = async (id: number) => {
        if(confirm('¿Estás seguro de eliminar este producto? Dejará de ser visible para los usuarios.')) {
            try {
    ...
    if (loading) return <div className="text-center py-10">Cargando {categoryName}...</div>;

    return (
        <div 
            id={sectionId}
            className='mb-12 mx-5 px-4 py-8 bg-white shadow-md rounded-xl border border-gray-100 scroll-mt-32'
        >
            ...
            {productos.length === 0 ? (
                <p className="text-gray-500 text-center">No hay productos registrados en esta categoría.</p>
            ) : (
        ...
        <div className="bg-gray-50 min-h-screen flex flex-col">
            ...
            <div className="sticky top-0 z-20 bg-white shadow-sm border-b border-gray-200">
                <div className="container mx-auto px-4 py-3 flex justify-between items-center">
                    ...
                    <ul className='flex flex-row gap-6 overflow-x-auto min-w-0 flex-1 mr-4 no-scrollbar pb-2 md:pb-0'>
                        {categorias.map((cat) => (
                    ...
                    <button 
                        ...
                        className="bg-green-600 hover:bg-green-700 text-yellow-300 font-bold py-2 px-6 rounded-full shadow-md transition-transform hover:scale-105 flex items-center gap-2 whitespace-nowrap flex-shrink-0"
                    >
